...
class CartListView extends Component {
			...
			if (nextProps.myCart && nextProps.globals.loading === false) {
				if (nextProps.myCart.update_qty_response && nextProps.myCart.update_qty_response.message && updateItemQty) {
					...
					this.setState({
						...
						addMessage:
							nextProps.globals.currentStore == 2 ? " Cart updated Successfully " : "تم تحديث عربة التسوق بنجاح ",
					})
	...
	removeFromWishlist = (product) => {
		product.wishListId = wishlistIds[wishlistProducts.indexOf(product.id)]
	...
	render() {
		const { myCart, globals, user_details } = this.props
							...
							<div className="row">
								<div className="col-md-8">
									<div className="cart-title-top">
										...
										{user_details.isUserLoggedIn && (
											<Link to={`/${globals.store_locale}/wishlist`}>
												<span className="view-wishlist">
													<FormattedMessage id="cart.viewWishList" defaultMessage="View Wish List" />{" "}
													{`(${wishlistUpdated.wishlistCount})`}
												</span>
											</Link>
										)}
									</div>
										...
										{myCart.products &&
											...
											products.map((product, index) => {
												return (
													<>
														{product.price === 0 && (
															<div className="productDescLine">
																<div className="row">
																	<div className="descBackgroudPro">
																		<img className="img-left" src={product.image[0]} />
																		<p className="productDes">
																			<FormattedMessage
																				id="cart.freeDots"
																				defaultMessage="You've qualified for a FREE "
																			/>
										...
										{myCart.products &&
											...
											products.map((product, index) => {
												return (
													<>
														{product.price !== 0 && (
															<div className="grid-spaceing" key={index}>
																				...
																				{product.is_in_stock.status === 1 && (
																					<div className="QtyCounter">
																						{parseInt(product.qty) === 1 && (
																							<span className=" blur minus" style={{ cursor: "unset" }}>
																								<img src={minIcon} alt="minIcon" />
																							</span>
																						)}
																						{parseInt(product.qty) > 1 && (
																							<span
																								onClick={() => this.updateQty(product, product.qty - 1)}
																								className="minus">
																								<img src={minIcon} alt="minIcon" />
																							</span>
																						)}
																						<input value={product.qty} type="text" placeholder="0" />
																			...
																			<div className="col-5 col-md-7">
																				<ul className="list-inline main_Ul">
																					<li
																						...
																						onClick={() => this.onClickRemoveIcon(product, index)}>
																						<div className="limitDiv cursor-pointer">
																							<svg
																								width="17px"
																								height="22px"
																								viewBox="0 0 17 22"
																								data-di-rand="1587466544680"
																								data-tip={
																									globals.store_locale === "en"
																										? `Click to remove item from cart`
																										: " انقر لإزالة المنتج إلى سلة التسوق "
																								}>
																					...
																					<li className="list-inline-item">
																						<div className="limitDiv cursor-pointer">
																							{wishlistUpdated.wishlistLoader &&
																							wishlistUpdated.wishlistLoadingProductId === product.id ? (
							...
							<div className="modal-body">
								<div className="container-fluid nopadding">
									<div className="deleteModelpop">
										<p>
											<FormattedMessage id="cart.removePopup" defaultMessage="Are you sure to remove item from cart?" />
										</p>
