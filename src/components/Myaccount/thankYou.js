...
import {
  ...
  purchaseEvent,
} from '../utility/googleTagManager';
const ThankyouSidebar = React.lazy(() =>
  import('../Myaccount/thankyouSidebar')
);
...
class ThankYouPage extends Component {
    ...
    if (query.get('paytype') == 'COD') {
      this.props.orderJson({
        store_id: this.props.globals.currentStore
          ? this.props.globals.currentStore
          : query.get('store_id'),
        order_id: cryptrOrderId.decrypt(query.get('order_id')),
      });
      if (
        !this.props.user_details.isUserLoggedIn &&
        (this.props.guest_user.temp_quote_id === null ||
          this.props.guest_user.temp_quote_id === '')
      ) {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.order &&
      nextProps.order.order_number &&
      nextProps.order.status &&
      !isGTMcalled
    ) {
      let ecomArray = [];
      ...
      for (let i = 0; i < item.length; i++) {
        ecomArray.push({
          ...
          price:
            parseInt(item[i].special_price) &&
            parseInt(item[i].special_price) !== parseInt(item[i].price)
              ? parseFloat(item[i].special_price)
              : parseInt(item[i].price),
    ...
    if (
      nextProps.order &&
      nextProps.order.order_number &&
      nextProps.myCart.cart_count === 0 &&
      guest_quote_id &&
      !isGetCart &&
      query.get('paytype') !== 'COD'
    ) {
      isGetCart = true;
  ...
  render() {
    if (
      !this.props.order ||
      !this.props.order.order_status_details ||
      !this.props.order.order_status_details.order_status ||
        ...
        <>
          ...
          {createMetaTags(
            this.props.globals.store_locale === 'en'
              ? 'My Order Summary | Official LEGO® Online Store Saudi Arabia'
              : 'ملخص الطلب | متجر ليغو أونلاين الرسمي بالسعودية ',
            this.props.globals.store_locale === 'en'
            ...
            <div className="container">
               ...
               */}
                              ...
                              <ul className="list-inline">
                                ...
                                <li className="list-inline-item">
                                  <p className="detailsTitle">
                                    <FormattedMessage
                                      id="Check.thankYou.OrderReceipt"
                                      defaultMessage="Order Receipt"
                                    />
                                  </p>
                                  <p className="detailsTitle">
                                    <FormattedMessage
                                      id="Check.thankYou.ordernumber"
                              ...
                              <ul className="list-inline">
                                <li className="list-inline-item">
                                  ...
                                  <p className="dataTitle">
                                    ...
                                    {this.props.order &&
                                      this.props.order.shipping_address &&
                                      this.props.order.shipping_address.country}
                                    ,
                                    {this.props.order &&
                                      this.props.order.shipping_address &&
                                      this.props.order.shipping_address
                                ...
                                <li className="list-inline-item">
                                  ...
                                  <p className="dataTitle">
                                    {this.props.order &&
                                      ...
                                      this.props.order.billing_address
                                        .company &&
                                      this.props.order.billing_address.company +
                                        ','}{' '}
                                    {this.props.order &&
                                      this.props.order.billing_address &&
                                      this.props.order.billing_address
                                    ...
                                    {this.props.order &&
                                      this.props.order.billing_address &&
                                      this.props.order.billing_address.city}
                                    ,{' '}
                                    {this.props.order &&
                                      this.props.order.billing_address &&
                                    ...
                                    {this.props.order &&
                                      this.props.order.billing_address &&
                                      this.props.order.billing_address.postcode}
                                    ,
                                  </p>
                                  <p className="dataTitle">
                                    <FormattedMessage
                            ...
                            <div className="statusShiped">
                              <ul className="list-inline">
                                <li className="list-inline-item">
                                  <p className="mainTitle">
                                    <FormattedMessage
                                      id="check.DeliveryType"
                                      defaultMessage="Delivery Type"
                                    />
                                  </p>
                                  <p className="dataTitle">
                                    {this.props.order &&
                                      this.props.order.delivery_type &&
                                      this.props.order.delivery_type}
                                  </p>
                                </li>
                              </ul>
                    ...
                    <div className="col-12 text-center mb-5">
                      <Link to={`/${this.props.globals.store_locale}`}>
                        <button className="ctnShopping">
                          <FormattedMessage
                            id="checkoutOrderSummary.ContinueShopping"
                  ...
                  <div className="col-md-4">
                    <ThankyouSidebar
                      order_summary={this.props.order.order_summary}
                      product_data={this.props.order.product_details}
                    />
                  </div>
                </div>
              </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    setShippingDetails: (payload) =>
      dispatch(actions.setShippingDetails(payload)),
    setBillingDetails: (payload) =>
      dispatch(actions.setBillingDetails(payload)),
    orderJson: (data) => dispatch(actions.orderJson(data)),
    updateQuoteID: (guest_quote_id) =>
      dispatch(actions.updateQuoteID(guest_quote_id)),
    getGuestCartId: (guest_quote_id) =>
      dispatch(actions.getGuestCartId(guest_quote_id)),
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(injectIntl(ThankYouPage)));
