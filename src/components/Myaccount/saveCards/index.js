...
const AddBagAlert = React.lazy(() =>
  import('../../../common/AlertBox/addToBagAlert')
);
const MyAccSideBar = React.lazy(() => import('../../Myaccount/MyAccSideBar'));

export default function SaveCards({ history }) {
  const state = useSelector((state) => state);

  const [show, setShow] = useState(false);

  const dispatch = useDispatch();
  const store_id = state && state.global && state.global.currentStore;
  const quote_id =
    state &&
    state.login &&
    state.login.customer_details &&
    state.login.customer_details.quote_id;
  const cards =
    state &&
    state.myCart &&
    state.myCart.payment_details &&
    state.myCart.payment_details.payfort_fort_cc &&
    state.myCart.payment_details.payfort_fort_cc.cards;
  const store_locale = state && state.global && state.global.store_locale;
  const deleteCardMsg = state && state.myCart && state.myCart.deleteCardMsg;
  ...
  useEffect(() => {
    try {
      if (document.getElementById('saveCards')) {
        document.getElementById('saveCards').classList.add('ActiveClass');
      }
    } catch (err) {}
  }, [dispatch, cards]);
  ...
  const logout = () => {
    dispatch(logoutUser());
    history.push(`/${store_locale}/home`);
  };

  const checkCardType = (num) => {
    switch (num.charAt(0)) {
      case '4':
        return 'visa';
      case '5':
  ...
  const getExpiryDate = (date) => {
    if (date)
      date = `${date.charAt(2)}${date.charAt(3)}/${date.charAt(0)}${date.charAt(
        1
      )}`;
  ...
  return (
    <div>
      {deleteCardMsg && deleteCardMsg !== '' && (
        <AddBagAlert
          message={deleteCardMsg}
          alertBoxStatus={true}
          closeBox={closeAddBag}
        />
      )}
      {createMetaTags(
        store_locale === 'en'
          ? 'My Cards | Official LEGO® Online Myord Saudi Arabia'
      ...
      <div className="row">
        ...
        <div className="col-md-9 pdL">
          <div className="rightSideContent save_cards">
            <div className="MyOrder">
              <div className="row">
                <div className="col-md-6 col-6">
                  <p className="accountTitle">
                    <FormattedMessage
                      id="myAccountInfo.saveCards"
                    ...
                    return (
                      <>
                        <div className="row pad10">
                          <div className="col-md-3 col-2 col-sm-2 col-lg-3">
                            {checkCardType(item.card_number) === 'mada' ? (
                              <span>
                                <img
                                  src={madacards}
                            ...
                            ) : checkCardType(item.card_number) === 'visa' ? (
                              <svg
                            ...
                            ) : checkCardType(item.card_number) ===
                              'mastercard' ? (
                              <svg
                          ...
                          <div className="col-md-3 col-5 col-sm-5 col-lg-3 card-number dir-ltr">
                            {item.card_number}
                          </div>
                          <div className="col-md-3 col-3 col-sm-3 col-lg-3 expiry-num">
                            {getExpiryDate(item.expiry_date)}
                          </div>
                          <div className="col-md-3 col-2 col-sm-2 col-lg-3">
                            <i
                                  ...
                                  <div className="deleteModelpop">
                                    ...
                                    <div className="ButtonDiv">
                                      <button
                                        ...
                                        onClick={hideModal}>
                                        <FormattedMessage
                                          id="myAccountInfo.Cancel"
                                          defaultMessage="Cancel"
                                        />
                                      </button>
                                      <button
                                        className="okbtn"
                                        onClick={() => deleteSavedCard(item.id)}>
                                        <FormattedMessage
