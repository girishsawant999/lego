...
class mainplp extends Component {
    ...
    componentWillMount() {
        ...
        if(this.props.location.pathname.includes('/productlisting/')){
            const payload = {
                ...
                url_key: url,
                page: 1,
                customerid: this.props.user_details.isUserLoggedIn ? this.props.user_details.customer_details.customer_id : ''
            }
    ...
    callPage = () => {
        ...
        if (page < this.props.plpData.total_page_count && !this.props.PlpNextLoader) {
            const payload = {
                store_id: store,
                url_key: url,
                page: page + 1,
    ...
    render() {
        ...
        if (this.props.plpSearch && this.props.searchLoader === false && this.props.plpSearch.searchWord && Object.values(this.props.product).length < 1 ){
            return (
            <div className="mainplp">
					{/* <LogoSlider /> */}
                    <div className="mt-4"><SearchNotFound searchKeyword={this.props.plpSearch.searchWord}/></div>
			...
			return (
				<div className="mainplp">
                    ...
                    <div className="pplPage">
                        {this.props.PlpCategory.toUpperCase()}
                    </div>
					<div className="container"><p className="CommingSoon"><span>{this.props.PlpMessage}</span></p></div>
				</div>
			)
        }
        ...
        return (
            <div>
                {createMetaTags(this.props.plpData.meta_title,
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(mainplp));