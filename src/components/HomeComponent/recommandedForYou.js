...
class Recommanded extends Component {
	...
	render() {
		...
		let products = data.recommended_data ? data.recommended_data : []
				...
				<div className="RecommendedPage">
					<div className="container">
						<p className="headTitle">
							<FormattedMessage id="RecommendedForYou" defaultMessage="Recommended For You" />
						</p>
									...
									Object.values(products).map((item, index) => (
										<div key={index} className="borderOuter productItemHome">
											...
											<div className="productdetails">
												<span className="p_new service-tag">
													{" "}
													<FormattedMessage id="Newtext" defaultMessage="New" />
												</span>
												<div className="bottomAdjust row-eq-height">
													<Link to={`/${store_locale}/productdetails/${item.url_key}`}>
														<p className="productTitle reco_ellipsis">{item.name}</p>
													</Link>
												</div>
												...
												{item.quantity_and_stock_status.is_in_stock ? (
													...
													<button className="btn-disabled" onClick={() => this.notifyMe(item.product_id)}>
														...
														{this.props.notify &&
															...
															this.state.notifiedProduct === item.product_id && (
