...
class FeatureSlider extends Component {
	...
	render() {
		...
		if(products && wishList.products) {
			var i = 0;
			let wishListProd = wishList.products
			for(var j = 0 ; j<products.length ; j++){
				for( i = 0 ; i<wishListProd.length ; i++){
					if(products[j].sku === wishListProd[i].sku){
						products[j].isInWishList = true;
						products[j].wishListId = wishListProd[i].wishlist_id
						break;
					}
					else{
						products[j].isInWishList = false;
					}
				}
			}
			...
			<div>
				...
				<div className="featureSet">
					<div className="container">
						<p className="headTitle"> <FormattedMessage id="Featured.sets" defaultMessage="Featured sets" /></p>
								...
								{	products &&
									products.map((product , index) => {
										return (
											...
											:
											<img onClick={() => this.addToWishList(product)} 
											...
											<li className="list-inline-item">
												<Link to="">
													...
													<p className="productNumber mb-1">{product.sku}</p>
													<p className="productPrice">{product.currency} {product.product_price}</p>												
													...
													<button className="btn-disabled" onClick={() => this.notifyMe(product.product_id)}>
														...
														{this.props.notify && this.props.notify.notifyLoader && this.state.notifiedProduct === product.product_id && <i class="fa fa-spinner fa-spin ml-1 mr-1" aria-hidden="true"></i>}
													</button> 
													}
