...
import {
  AddToCartEvent,
  RemoveProductCart,
} from '../../components/utility/googleTagManager';
import * as actionType from '../actionTypes';
import { loadingSpinner } from './globals';
import { removeWishListUpdated } from './wishlistUpdated';
...
export const addToCart = (
  ...
  return (dispatch, getState) => {
    ...
    let cb = {
      success: (res) => {
        ...
        if (removeFromWishlist !== null) {
          dispatch(removeWishListUpdated(removeFromWishlist));
        }
      },
      error: (err) => {
        dispatch(
    ...
    API.addToCart(data, cb, getState().global.currentStore, getState().global.store_locale);
  };
};
...
export const addToCartFromWishList = (payload, myCart, eventObj) => {
  return (dispatch, getState) => {
    ...
    dispatch(
      callActionWishlistitem_added({
        addToCardLoding: false,
        wishlistaddToCartPopop: false,
        item_added: false,
        add_cart_error: false,
        add_cart_open_popUp: true,
      })
    ...
    let cb = {
      success: (res) => {
        if (res.status && res.code === 200 && 'data' in res) {
          ...
          dispatch(loadingSpinner({ loading: false }));
          dispatch(
            callActionForGetCartCount({ my_cart_count: res.data.cart_count })
          );
          if (isCallPayment || getState().myCart.my_cart_count <= 1) {
            dispatch(
              getPaymentDetailsAfterVou({
    ...
    let cb = {
      success: (res) => {
        if (res.status && res.code === 200) {
          ...
          dispatch(
            callActionForGetPaymentDetails({
    ...
    let cb = {
      success: (res) => {
        if (res.status) {
          ...
          dispatch(
            changeQtyState({ products: prodArray, update_qty_response: data })
          );
          dispatch(
            getMyCart({
              quote_id: getState().myCart.quote_id,
    ...
    let cb = {
      success: (res) => {
        if (res.status) {
            ...
            product: [
              {
                ...
                id: removeProd.sku,
                price: removeProd.special_price
                  ? removeProd.special_price
                  : removeProd.price,
                brand: 'Google',
                category: '',
          ...
          if (getState().myCart.products && getState().myCart.products.length === 0 && 
          getState().myCart.voucher && getState().myCart.voucher !== '') {
            dispatch({
              type: actionType.SET_VOU_CODE,
              payload: {
                voucher: '',
