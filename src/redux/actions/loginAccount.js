...
import { getWishlistUpdated } from './wishlistUpdated';
...
export const registerUser = (payload) => {
  return (dispatch) => {
    ...
    dispatch(
      callActionRegisterUser({ registerUserDetails: {}, registerLoader: true })
    );
    let cb = {
      success: (res) => {
  ...
  return (dispatch) => {
    ...
    let cb = {
      success: (res) => {
        if (res.status === true && res.code === 200) {
          dispatch(
            callActionForgotPassword({ forgotPasswordDetails: { ...res } })
          );
        } else {
          dispatch(
            callActionForgotPassword({ forgotPasswordDetails: { ...res } })
    ...
    let cb = {
      success: (res) => {
        if (res.status === true && res.code === 200) {
          ...
          dispatch(getWishlistUpdated({ ...wishdata }));
          dispatch(getMyCart(payload));
        } else {
          let newState = {
            isUserLoggedIn: res.status,
