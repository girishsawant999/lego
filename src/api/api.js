...
const SET_CONTACT_US_DATA = {
  type: 'POST',
  url: BASE_URL + 'setContactUsData',
};
const GET_HOME_PAGE_DATA = { type: 'GET', url: UPDATED_BASE_URL + 'homepage' };
...
const GET_ORDER_DETAILS_IN_PROFILE = {
  type: 'POST',
  url: BASE_URL + 'orderview/',
};
...
const GET_PAYMENT_DETAILS_CHECKOUT = {
  type: 'POST',
  url: BASE_URL + 'getpayment/',
};
const SET_PAYMENT_DETAILS_CHECKOUT = {
  type: 'POST',
...
const PAYFORT_RESTORE = { type: 'POST', url: BASE_URL + 'payfortrestorequote/' };
const DELETE_SAVED_CARD = { type: 'POST', url: BASE_URL + 'deleteCardById/' };

export const API = {
  getCartCount: (data, cb) => request(data, cb, GET_CART_COUNT),
  addToCart: (data, cb, store_code, storeLocale) => {
    let ADD_TO_CARD_API =
      CART_URL + storeLocale + '/V1/carts/mine/items?storeid=' + store_code;
    request(data, cb, { type: 'POST', url: ADD_TO_CARD_API });
  },
  getGuestCartId: (data, cb) => request(data, cb, GET_GUEST_CART_ID),
  guestAddToCart: (data, cb, store_code,storeLocale) => {
    let GUEST_ADD_TO_CART_LINK =
      GUEST_CART_URL + storeLocale +
      '/V1/guest-carts/' +
      data.cart_item.quote_id +
      '/items?storeid=' +
      store_code;
  ...
  getOrderDetailsInProfile: (data, cb) =>
    request(data, cb, GET_ORDER_DETAILS_IN_PROFILE),
  ...
  getPlpData: (data, cb) => {
    let GET_PLP_DATA = `${UPDATED_BASE_URL}productlisting/`;
    request(data, cb, { type: 'GET', url: GET_PLP_DATA });
  },
  searchResult: (data, cb) => {
    let SEARCH_RESULT = `${BASE_URL}searchresult`;
  ...
  changePassword: (data, cb, customerId) => {
    const CHANGE_PASSWORD_KEY = {
      type: 'PUT',
      url: BASE_URL_1 + 'customers/me/password' + '?customerId=' + customerId,
    };
  ...
  getPaymentDetails: (data, cb) =>
    request(data, cb, GET_PAYMENT_DETAILS_CHECKOUT),
  setPaymentDetails: (data, cb) =>
    request(data, cb, SET_PAYMENT_DETAILS_CHECKOUT),
  ...
  payfortRestoreQuote: (data, cb) => request(data, cb, PAYFORT_RESTORE),
  deleteSavedCard: (data, cb) => request(data, cb, DELETE_SAVED_CARD)
};
